...
from levelXP import ReputationSystem
...
def create(msg):
    ...
    text = msg.text[9:]
    response = todo.route(uid, text, 'create')
...
def read(msg):
    uid = msg.from_user.id
    text = msg.text[9:]
    response = todo.route(uid, text, 'read')
    bot.reply_to(msg, response)

@bot.message_handler(commands=['rmvnote'])
def delete(msg):
    uid = msg.from_user.id
    text = msg.text[9:]
    response = todo.route(uid, text, 'delete')
    bot.reply_to(msg, response)
                ...
                if text_len < 40:
                    ...
                    if start:
                        ...
                        if msg.reply_to_message != None:
                            ...
                            if to_member.user.id==6022558336 or to_member.user.id==6123493512 or to_member.user.id==6201902583:
                            ...
                            if superuser:
                                if lst[0] == '/superrank':
                                    ...
                                    message = f"A superuser has changed rep to [{a(chat_id, lst[1])}] for {bold(username)}"
                                    prev_msg_id = bot.send_message(chat_id, message, parse_mode='HTML', disable_web_page_preview=True).message_id
                                    del_msg('super')
                                    delete_command(chat_id, msg_id)
                                    crud.data[chat_id]['last_message_id']['super'] = prev_msg_id
                                    ...
                                    message = f"A superuser has changed level to [{a(chat_id, lst[1])}] for {bold(username)}"
                                    prev_msg_id = bot.send_message(chat_id, message, parse_mode='HTML', disable_web_page_preview=True).message_id
                                    del_msg('super')
                                    delete_command(chat_id, msg_id)
                                    crud.data[chat_id]['last_message_id']['super'] = prev_msg_id
